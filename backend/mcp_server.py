"""
DocuMentor MCP Wrapper — v0.3.1
---------------------------------
Exposes SurfSense as MCP tools via JSON-RPC.

Endpoints:
  - POST /mcp      — JSON-RPC (primary)
  - POST /jsonrpc   — JSON-RPC (alias for bridge compatibility)
  - GET  /health    — Health check
  - GET  /tools     — Debug tool listing

Changes from v0.3.0:
  - Added /jsonrpc alias (bridge expects /jsonrpc, not /mcp).
  - Fixed query endpoint: POST /api/v1/new_chat (was /threads/{id}/messages which
    is for persisting messages, not for chat/streaming).
  - Default SURFSENSE_BASE_URL → http://backend:8000 (Docker network).
  - Default SURFSENSE_EMAIL → admin@documenter.app (.local is reserved TLD).
"""
...
async def jsonrpc_endpoint(request: Request) -> JSONResponse:
    """Legacy JSON-RPC endpoint — bridge calls /jsonrpc, alias of /mcp."""
